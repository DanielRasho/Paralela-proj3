...
// ==========================================
// CONSTANT MEMORY declarations (must be global scope)
__constant__ float d_Cos[180 / 2];  // degreeBins = 90
__constant__ float d_Sin[180 / 2];  // degreeBins = 90
...
//==============================
// GPU kernel - NOW using CONSTANT MEMORY (d_Cos and d_Sin are global)
                               ...
                               float rMax, float rScale)
{
  ...
  if (pic[gloID] > 0)
  {
    ...
    {
      // Calculate rho using precomputed sin/cos values FROM CONSTANT MEMORY
      ...
      {
        // Atomic add because multiple threads may write to same bin
...
int main (int argc, char **argv)
{
  ...
  printf("==== CUDA Hough Transform (CONSTANT Memory) ====\n");
  ...
  // Allocate device memory (NO more d_Cos and d_Sin with cudaMalloc!)
  unsigned char *d_in;
  int *d_hough;
  ...
  // Launch kernel (NO d_Cos and d_Sin parameters!)
  ...
  // Cleanup (NO cudaFree for d_Cos and d_Sin - they are constant memory!)
  free(outputImage);
  free(h_hough);
  free(h_Cos);
  free(h_Sin);
