                               ...
                               float rMax, float rScale, float *d_Cos, float *d_Sin)
{
  ...
  if (pic[gloID] > 0)
  {
    ...
    {
      // Calculate rho using precomputed sin/cos values
...
int main (int argc, char **argv)
{
  ...
  printf("==== CUDA Hough Transform (Global Memory) ====\n");
  ...
  GPU_HoughTran<<<gridSize, blockSize>>>(d_in, w, h, d_hough, rMax, rScale, d_Cos, d_Sin);
